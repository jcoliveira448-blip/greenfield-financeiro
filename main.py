...
if not df_medicoes_global.empty and 'valor' in df_medicoes_global.columns:
    ...
    saldo_projetado_caixa = 1250000.00  # Caso não haja medições, inicia com a base padrão
...
# ===================== 3. PEDIDOS DE COMPRA =====================
    ...
    with aba3:
        ...
        if not df_ger.empty:
            ...
            if st.button("💾 Sincronizar Status da Compra"):
                ...
                for idx, row in mudancas.iterrows():
                    up_data = {"solicitante": str(row['solicitante']), "fornecedor": str(row['fornecedor']), "valor_total": float(row['valor_total']), "status": str(row['status'])}
...
# ===================== 4. ACORDOS JUDICIAIS =====================
    ...
    with aba3:
        ...
        if not df_ger.empty:
            ...
            mudancas = st.data_editor(df_ger, use_container_width=True, num_rows="dynamic", hide_index=True, key="edit_aj", column_config={"valor_parcela": st.column_config.NumberColumn("Valor da Parcela (R$)", format="R$ %.2f")})
...
# ===================== 5. SALÁRIOS =====================
    ...
    with aba3:
        ...
        if not df_ger.empty:
            ...
            mudancas = st.data_editor(df_ger, use_container_width=True, num_rows="dynamic", hide_index=True, key="edit_sl", column_config={"salario_base": st.column_config.NumberColumn("Salário Base (R$)", format="R$ %.2f")})
